...
# from modules.sensor import SensorDetector  # Commented out sensor functionality
...
def main():
    try:
        ...
        # sensor = SensorDetector()  # Commented out sensor initialization
        ...
        while True:
            camera_result = camera.detect()  # Direct detection call
            # sensor_result = sensor.get_detection_result()  # Commented out sensor detection
            current_time = time.time()
            
            if current_time - last_detection_time >= detection_cooldown:
                # if camera_result["human_detected_by_cam"] or sensor_result["human_detected_by_sensor"]:  # Original line
                if camera_result["human_detected_by_cam"]:  # Modified to only use camera
                    logger.info(f"Human detected - Camera: {camera_result['human_detected_by_cam']}")
